...
import type { BossId, EnemyArchetypeId, EnemyMovementPattern, StageDefinition, StageEnemyEvent } from '../game/types'
import { DEFAULT_BOSS_ID, getBossDefinition } from './bosses'
...
export function createDemoStage(bossId: BossId = DEFAULT_BOSS_ID): StageDefinition {
  const boss = getBossDefinition(bossId)
  return {
    id: 'demo-stage-1',
    title: boss.theme.stage,
    subtitle: boss.theme.spell,
    starCount: 112,
    bossAppearMs: 92_000,
    boss,
    score: SCORE_MODEL,
    events: DEMO_STAGE_EVENTS,
  }
}

export const DEMO_STAGE: StageDefinition = createDemoStage(DEFAULT_BOSS_ID)
